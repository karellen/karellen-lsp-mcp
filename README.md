...
| Language | Default LSP Server | Install |
|----------|-------------------|---------|
| C | [clangd](https://clangd.llvm.org/) | `dnf install clang-tools-extra` / `apt install clangd` |
| C++ | [clangd](https://clangd.llvm.org/) | `dnf install clang-tools-extra` / `apt install clangd` |
| Any | Custom via `lsp_command` parameter | Provide the command in `lsp_register_project` |

For C/C++ projects, clangd works best with a `compile_commands.json` file (generated by
CMake, Meson, Bear, etc.). Pass the directory containing it via the `build_info` parameter.
...
The daemon stores its files at:

- **Linux/macOS**: `~/.local/share/karellen-lsp-mcp/`
- **Windows**: `%LOCALAPPDATA%/karellen-lsp-mcp/`

| File | Purpose |
|------|---------|
| `daemon.sock` | Unix domain socket for frontend connections |
| `daemon.lock` | Lock file preventing multiple daemon instances |
| `daemon.log` | Daemon log file (startup, connections, errors, LSP progress) |
